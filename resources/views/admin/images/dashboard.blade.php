                            ...
                            @for($i = 1; $i <= $buttons; $i++)
                                @if($buttons > 30)
                                    @if($i<=8 or ($pageid+5 >= $i and $pageid-5 <= $i) or $i >= ($buttons-8))
                                        <button type="button" onclick="location.href='{{route('AdminImages', ['pageid' => $i])}}';" class="btn btn-dark">{{ $i }}</button>
                                    @endif
                                @else
                                    <button type="button" onclick="location.href='{{route('AdminImages', ['pageid' => $i])}}';" class="btn btn-dark">{{ $i }}</button>
                                @endif
                            ...
                            @foreach($images as $image)
                                ...
                                @else
                                    <div class="card mt-2 mr-1" style="width: 18rem;">
                                        ...
                                        <div class="card-body mx-auto">
                                            <a href="{{route('deleteImage', ['imageid' => $image->id])}}" class="btn btn-danger">
                                                Delete
                                            </a>
                                        </div>
                                        <div class="mx-auto">
